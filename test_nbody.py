...
from examples.nbody import World, evolve, evolve_c_simple1, evolve_c_simple2
...
def display_error(fn, name, steps=1000, dt=1e-3, bodies=101, threads=1):
    ...
    w1 = World(10, threads=threads, dt=dt)
    w2 = w1.copy()
    evolve(w1, 1024)
    fn(w2, 1024)
    def f(name):
...
def timer(fn, name, steps=1000, dt=1e-3, bodies=101, threads=1):
    ...
    w = World(bodies, threads=threads, dt=dt)
    t0 = time.time()
    fn(w, steps)
    t1 = time.time()
    ...
    # compare the result to the pure python result 
    display_error(fn, name, steps, dt, bodies, threads)

    

if __name__ == "__main__":
    timer(evolve, "Python", steps=1000)
    timer(evolve_c_simple1, "C Simple 1", steps=32000)
    timer(evolve_c_simple2, "C Simple 2", steps=32000)
